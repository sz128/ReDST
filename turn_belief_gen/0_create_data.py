...
def getDialogueAct(filename, data, data2, idx, idx_acts):
    ...
    if not isinstance(turn, str): # and not isinstance(turn, unicode):
        for k in turn.keys():       ###########################################################只考虑inform和request? 而且预测部分只有inform？！
...
def get_summary_bstate(bstate, get_domain=False):
    ...
    assert len(summary_bstate) == 94  # book info + slot + domain active signal = 94
    if get_domain:
...
def createData():
    ...
    for didx, dialogue_name in enumerate(data):
        ...
        for idx, turn in enumerate(dialogue['log']):
            ...
            dialogue = fixDelex(dialogue_name, dialogue, data2, idx, idx_acts)  # this part might not be useful anymore
    ...
    for dialogue_name in data:
        ...
        if dial:
            ...
            for turn_i, turn in enumerate(dial):
                ...
                turn_dialog['belief_state'] = [{"slots": [s], "act": "inform"} for s in turn['bvs']] ## ############################################################## always inform????
                turn_dialog['turn_label'] = [bs["slots"][0] for bs in turn_dialog['belief_state'] if bs not in last_bs]   # 去掉重复，只看当前turn的新slotvalue
