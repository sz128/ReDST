...
def collate_fn(data):
    ...
    system_acts_mask = (torch.tensor(system_acts_sent_seq_lengths) != 0)
    ...
    if USE_CUDA:
        ...
        system_acts_mask = system_acts_mask.long().cuda()
    ...
    item_info["system_acts_mask"] = system_acts_mask
...
#            last_domain = ''
            ...
            for ti, turn in enumerate(dial_dict["dialogue"]):
                ...
                system_acts_sent = turn["system_acts_sent"].strip()
...
def dump_pretrained_emb(word2index, index2word, dump_path):
    ...
    json.dump(word2index, open("./data/word2index.json", "w"))
    json.dump(index2word, open("./data/index2word.json", "w"))
    print("dump index2word/word2index finish")
