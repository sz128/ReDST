...
class Dataset(data.Dataset):
    ...
    def __init__(self, data_info, src_word2id, trg_word2id, sequicity, mem_word2id):
        ...
        self.last_turn_belief=data_info['last_turn_belief']
        self.last_turn_gating_label=data_info['last_turn_gating_label']
        self.last_turn_domain=data_info['last_turn_domain']
        self.last_turn_generate_y=data_info['last_turn_generate_y']
        self.ti=data_info['ti']
    ...
    def __getitem__(self, index):
        ...
        last_turn_belief=self.last_turn_belief[index]
        gating_label = self.gating_label[index]
        last_turn_gating_label=self.last_turn_gating_label[index]
        turn_uttr = self.turn_uttr[index]
        turn_uttr_ids=self.preprocess(turn_uttr,self.src_word2id)
        turn_domain = self.preprocess_domain(self.turn_domain[index])
        last_turn_domain=self.preprocess_domain(self.last_turn_domain[index])
        generate_y = self.generate_y[index]
        generate_y = self.preprocess_slot(generate_y, self.trg_word2id)
        last_turn_generate_y=self.last_turn_generate_y[index]
        last_turn_generate_y=self.preprocess_slot(last_turn_generate_y,self.trg_word2id)
        ...
        item_info = {
            ...
            'last_turn_gating_label':last_turn_gating_label,
            'turn_uttr':turn_uttr_ids,
            'last_turn_domain':last_turn_domain,
            'last_turn_generate_y':last_turn_generate_y,
            'ti':ti
            }
        return item_info
...
def collate_fn(data):
    ...
    last_y_seqs,last_y_lengths=merge_multi_response(item_info['last_turn_generate_y'])
    gating_label = torch.tensor(item_info["gating_label"])
    turn_domain = torch.tensor(item_info["turn_domain"])
    ti=torch.tensor(item_info['ti'])

    current_uttr_seqs,current_uttr_lengths=merge(item_info['turn_uttr'])
    last_turn_gatting_label=torch.tensor(item_info['last_turn_gating_label'])
    # last_turn_domain=torch.tensor(item_info['last_turn_domain'])
    # last_turn_belief=torch.tensor(item_info['last_turn_belief'])

    if USE_CUDA:
        src_seqs = src_seqs.cuda()
        ti=ti.cuda()
        current_uttr_seqs=current_uttr_seqs.cuda()
        ...
        last_turn_gatting_label=last_turn_gatting_label.cuda()
        # last_turn_belief=last_turn_belief.cuda()
    ...
    item_info['last_turn_generate_y']=last_y_seqs
    item_info['last_y_lengths']=last_y_lengths

    item_info['turn_uttr']=current_uttr_seqs
    item_info['turn_len']=current_uttr_lengths
    item_info['last_turn_gating_label']=last_turn_gatting_label
    # item_info['last_turn_domain']=last_turn_domain
    return item_info
...
def read_langs(file_name, gating_dict, SLOTS, dataset, lang, mem_lang, sequicity, training, max_line = None):
    ...
    with open(file_name) as f:
        ...
        for dial_dict in dials:
            ...
            添加上一轮次的信息
            """
            last_turn_belief=None
            last_turn_gating_label=None
            last_turn_domain=None
            last_turn_generate_y=None
            ...
            for ti, turn in enumerate(dial_dict["dialogue"]):
                ...
                data_detail = {
                    ...
                    'generate_y':generate_y,
                    'last_turn_generate_y':last_turn_generate_y if last_turn_generate_y!=None else generate_y,
                    'last_turn_belief':last_turn_belief if last_turn_belief!=None else turn_belief_list,
                    'last_turn_gating_label':last_turn_gating_label if last_turn_gating_label!=None else gating_label,
                    'last_turn_domain':last_turn_domain if last_turn_domain!=None else turn_domain
                    }
                data.append(data_detail)
                last_turn_belief=turn_belief_list
                last_turn_gating_label=gating_label
                last_turn_domain=turn_domain
                last_turn_generate_y=generate_y
...
def prepare_data_seq(training, task="dst", sequicity=0, batch_size=100):
    ...
    if training:
        print('start to read lang-train')
