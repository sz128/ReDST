...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        turn_generate_y = self.preprocess_slot_bert(turn_generate_y)
        ...
        current_generate_y = self.preprocess_slot_bert(current_generate_y)
        ...
        last_generate_y = self.preprocess_slot_bert(last_generate_y)

        context = self.dialog_history[index] 
        context = self.preprocess(context, self.src_word2id)
        context_plain = self.dialog_history[index]
        turn_uttr = self.preprocess_bert(self.turn_uttr[index])
        tmp = self.type_ids[index]
        turn_uttr_type_ids = torch.Tensor([0] * tmp[0] + [1] * tmp[1])

        system_acts_sent= self.preprocess_bert(self.system_acts_sent[index])
        system_acts_sent_type_ids = torch.Tensor([0] * self.system_acts_sent_type_ids[index][0])
        ...
        item_info = {
            ...
            "turn_uttr":turn_uttr,
            "turn_uttr_type_ids":turn_uttr_type_ids,
    ...
    def preprocess_slot_bert(self, sequence):
        """Converts words to ids."""
        story = []
        for value in sequence:
            v = tokenizer.convert_tokens_to_ids(value.split() + ["[SEP]"])
            story.append(v)
        # story = torch.Tensor(story)
        return story
...
def collate_fn(data):
    ...
    def merge_bert(sequences, type_ids):
        ...
        return padded_seqs, padded_type_ids, lengths
    ...
    turn_uttr_src_seqs, turn_uttr_src_type_ids, turn_uttr_src_lens = merge_bert(item_info['turn_uttr'], item_info['turn_uttr_type_ids'])
    turn_uttr_mask = (turn_uttr_src_seqs != 0) # generate mask for input sequence

    system_acts_sent_seqs, system_acts_sent_type_ids, system_acts_sent_lens = merge_bert(item_info['system_acts_sent'], item_info['system_acts_sent_type_ids'])
    system_acts_sent_mask = (system_acts_sent_seqs != 0) # generate mask for input sequence
    ...
    if USE_CUDA:
        ...
        turn_uttr_src_type_ids = turn_uttr_src_type_ids.long().cuda()

        system_acts_sent_seqs = system_acts_sent_seqs.cuda()
        system_acts_sent_mask = system_acts_sent_mask.long().cuda()
        system_acts_sent_type_ids = system_acts_sent_type_ids.long().cuda() 
    ...
    item_info["turn_uttr_len"] = turn_uttr_src_lens
    item_info["turn_uttr_type_ids"] = turn_uttr_src_type_ids

    item_info["system_acts_sent_seq"] = system_acts_sent_seqs
    item_info["system_acts_sent_mask"] = system_acts_sent_mask
    item_info["system_acts_sent_len"] = system_acts_sent_lens 
    item_info["system_acts_sent_type_ids"] = system_acts_sent_type_ids
...
def read_langs(file_name, gating_dict, SLOTS, dataset, lang, mem_lang, sequicity, training, max_line = None):
    ...
    with open(file_name) as f:
        ...
        #            lang.index_words(turn["system_transcript_bert"], 'utter')
        #            lang.index_words(turn["transcript_bert"], 'utter')
...
#            last_domain = ''
            ...
            for ti, turn in enumerate(dial_dict["dialogue"]):
                ...
                type_ids = [0, 0]
                type_ids[0] = len(turn["system_transcript_bert"].strip().split()) + 2
                type_ids[1] = len(turn["transcript_bert"].strip().split()) + 1
                turn_uttr = "[CLS] " + turn["system_transcript_bert"].strip() + " [SEP] " + turn["transcript_bert"].strip() + " [SEP]"

                system_acts_sent_type_ids = [0, 0]
                system_acts_sent_type_ids[0] = len(turn["system_acts_sent_bert"].strip().split()) + 2
                system_acts_sent = "[CLS] " + turn["system_acts_sent_bert"].strip() + " [SEP]"

                dialog_history +=  (turn["system_transcript_bert"] + " ; " + turn["transcript_bert"] + " ; ")
                ...
                for k, v in turn_label_dict.items():
                    turn_label_dict[k] = " ".join(tokenizer.tokenize(v))
                for k, v in turn_belief_dict.items():
                    turn_belief_dict[k] = " ".join(tokenizer.tokenize(v))
...
def prepare_data_seq(training, task="dst", sequicity=0, batch_size=100):
    ...
    if training:
        ...
        train = get_seq(pair_train, lang, mem_lang, batch_size, False, sequicity)
        ...
        if os.path.exists(folder_name+lang_name) and os.path.exists(folder_name+mem_lang_name):
            print("[Info] Loading saved lang files from %s ..." %(folder_name))
        ...
        emb_dump_path = 'data/emb{}.json'.format(len(tokenizer.vocab))
        if not os.path.exists(emb_dump_path) and args["load_embedding"]:
            ...
            dump_pretrained_emb_new(tokenizer, emb_dump_path)
    else:
        with open(folder_name+lang_name, 'rb') as handle:
            lang = pickle.load(handle)
        with open(folder_name+mem_lang_name, 'rb') as handle:
    ...
    print("Vocab_size: %s " % len(tokenizer.vocab))
    print("Max. length of dialog words for RNN: %s " % max_word)
    print("USE_CUDA={}".format(USE_CUDA))

    SLOTS_LIST = [ALL_SLOTS, slot_train, slot_dev, slot_test]
    print("[Train Set Slots]: Number is {} in total".format(str(len(SLOTS_LIST[1]))))
    print(SLOTS_LIST[1])
    print("[Dev Set Slots]: Number is {} in total".format(str(len(SLOTS_LIST[2]))))
    print(SLOTS_LIST[2])
